        ...
        <main class="main">
            <div class="topbar">
                <div class="page-title">
                    <h1>Exercises for Day {{ $planDay->day_number }}</h1>
                </div>
                <div class="user">
                ...
                @if (!$planDay->is_rest_day)
                    ...
                    <div class="card add-form-container" id="addFormContainer" style="margin-bottom:14px">
                        <div class="toolbar" style="margin-bottom:8px">
                            <div class="muted">Add New Exercise</div>
                        </div>
                        <form class="form-inline" action="{{ route('admin.plans.day.exercises.store', $planDay) }}"
                            method="post">
                            @csrf
                            <div class="field">
                                ...
                                <select name="exercies_level_id" required>
                                    ...
                                    @foreach ($availableExerciseLevels as $el)
                                        <option value="{{ $el->id }}">{{ $el->name }} ({{ $el->calories }}
                                            cal)</option>
                                    @endforeach
                                </select>
                            </div>
                        ...
                        <tbody>
                            @forelse($exercises as $index => $ex)
                                <tr>
                                    ...
                                    <td>
                                        @if ($ex->image_path)
                                            <img class="img" src="{{ asset('imgs/' . $ex->image_path) }}"
                                                alt="{{ $ex->name }}" />
                                        @else
                                            —
                                        @endif
                                    ...
                                    <td>
                                        <div class="actions">
                                            ...
                                            <form action="{{ route('admin.plans.day.exercises.delete', $ex->id) }}"
                                                method="post"
                                ...
                                <tr id="edit-row-{{ $ex->id }}" style="display:none;background:#f8fafc">
                                    <td colspan="8">
                                        <form class="form-inline"
                                            action="{{ route('admin.plans.day.exercises.update', $ex->id) }}"
                                            method="post">
                                            @csrf
                                            <div class="field">
                                                <label>Exercise</label>
                                                <select name="exercies_level_id" required>
                                                    @foreach ($availableExerciseLevels as $el)
                                                        <option value="{{ $el->id }}"
                                                            {{ $ex->exercies_level_id == $el->id ? 'selected' : '' }}>
                                                            {{ $el->name }} ({{ $el->calories }} cal)</option>
                                                    @endforeach
                                                </select>
                                            </div>
                                            <div class="field">
                                                <label>Order</label>
                                                <input type="number" name="exercies_order" min="1"
                                                    value="{{ $ex->exercies_order }}" required />
                                            </div>
                                            <div>
