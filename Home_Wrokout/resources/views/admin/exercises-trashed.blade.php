        ...
        <main class="main">
            ...
            <div class="page">
                <div class="add-button-container">
                    <a href="{{ route('admin.exercises') }}" class="btn gray"
                    ...
                    <div class="alert error" style="margin-bottom: 20px;">
                        <div style="display: flex; justify-content: space-between; align-items: center;">
                            ...
                            @if (session('show_cancel_button'))
                                <a href="{{ route('admin.exercises.trashed') }}" class="btn small gray"
                                ...
                                <tr>
                                    ...
                                    <td>
                                        @if ($exercise->image_path)
                                            <img src="{{ asset('imgs/' . $exercise->image_path) }}"
                                                alt="{{ $exercise->name }}"
                                    ...
                                    <td>{{ $exercise->deleted_at ? $exercise->deleted_at->format('Y-m-d H:i:s') : '—' }}
                                    </td>
                                    <td>
                                        <div class="actions">
                                            <form action="{{ route('admin.exercises.restore', $exercise->id) }}"
                                                method="post">
                                                @csrf
                                                <button class="btn small blue" type="submit"
                                                    onclick="return confirm('Restore this exercise?')">Restore</button>
                                            </form>
                                            <form action="{{ route('admin.exercises.force-delete', $exercise->id) }}"
                                                method="post">
                                                @csrf
                                                <button class="btn small red" type="submit"
                                                    onclick="return confirm('Permanently delete this exercise? This action cannot be undone!')">Delete
                                                    Forever</button>
                                            </form>
                                        </div>
                                    </td>
