...
<div class="layout">
    ...
    <main class="main">
        <div class="topbar">
            <div class="page-title"><h1>Deleted Plans</h1></div>
        </div>

        <div class="page">
            <!-- Back to Plans Button -->
            <div class="add-button-container">
                <a href="{{ route('admin.plans') }}" class="btn gray" style="text-decoration: none; display: inline-block;">
                    ← Back to Plans
                </a>
            </div>
                ...
                <div class="alert error" style="margin-bottom: 20px;">
                    <div style="display: flex; justify-content: space-between; align-items: center;">
                        ...
                        @if(session('show_cancel_button'))
                            <a href="{{ route('admin.plans.trashed') }}" class="btn small gray" style="text-decoration: none;">
                                Cancel
                            </a>
                        @endif
                    </div>
                ...
                <table class="table">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>{{ $plan->deleted_at ? $plan->deleted_at->format('Y-m-d H:i:s') : '—' }}</td>
                            <td>
                                <div class="actions">
                                    <form action="{{ route('admin.plans.restore', $plan->id) }}" method="post">
                                        @csrf
                                        <button class="btn small blue" type="submit" onclick="return confirm('Restore this plan?')">Restore</button>
                                    </form>
                                    <form action="{{ route('admin.plans.force-delete', $plan->id) }}" method="post">
                                        @csrf
                                        <button class="btn small red" type="submit" onclick="return confirm('Permanently delete this plan? This action cannot be undone!')">Delete Forever</button>
                                    </form>
                                </div>
                            </td>
                        </tr>
                        @empty
                        <tr>
                            <td colspan="7" class="muted">No deleted plans found.</td>
                        </tr>
                        @endforelse
                    </tbody>
                </table>
