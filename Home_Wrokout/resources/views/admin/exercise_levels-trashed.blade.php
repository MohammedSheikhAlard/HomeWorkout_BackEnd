        ...
        <main class="main">
            <div class="topbar">
                <div class="page-title">
                    <h1>Deleted Exercise Levels</h1>
                </div>
            </div>

            <div class="page">
                <div class="add-button-container">
                    <a href="{{ route('admin.exercise-levels') }}" class="btn gray"
                    ...
                    <div class="alert error" style="margin-bottom: 20px;">
                        <div style="display: flex; justify-content: space-between; align-items: center;">
                            ...
                            @if (session('show_cancel_button'))
                                <a href="{{ route('admin.exercise_levels.trashed') }}" class="btn small gray"
                            ...
                            @forelse($exerciseLevels as $index => $exerciseLevel)
                                <tr>
                                    ...
                                    <td>
                                        ...
                                        @else
                                            <span class="pill" style="background:#fee2e2;color:#991b1b">No
                                                Level</span>
                                        @endif
                                    </td>
                                    ...
                                    <td>{{ $exerciseLevel->deleted_at ? $exerciseLevel->deleted_at->format('Y-m-d H:i:s') : '—' }}
                                    </td>
                                    <td>
                                        <div class="actions">
                                            <form
                                                action="{{ route('admin.exercise-levels.restore', $exerciseLevel->id) }}"
                                                method="post">
                                                @csrf
                                                <button class="btn small blue" type="submit"
                                                    onclick="return confirm('Restore this exercise level?')">Restore</button>
                                            </form>
                                            <form
                                                action="{{ route('admin.exercise-levels.force-delete', $exerciseLevel->id) }}"
                                                method="post">
                                                @csrf
                                                <button class="btn small red" type="submit"
                                                    onclick="return confirm('Permanently delete this exercise level? This action cannot be undone!')">Delete
                                                    Forever</button>
                                            </form>
                                        </div>
                                    </td>
