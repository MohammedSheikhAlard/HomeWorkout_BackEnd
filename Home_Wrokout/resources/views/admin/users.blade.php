            ...
            <div class="page">
                @if (session('success'))
                    <div class="alert success"
                        style="background: #dcfce7; border: 1px solid #22c55e; color: #166534; padding: 12px; border-radius: 8px; margin-bottom: 16px;">
                        {{ session('success') }}
                    </div>
                @endif
                ...
                @if (session('error'))
                    <div class="alert error"
                        style="background: #fee2e2; border: 1px solid #ef4444; color: #991b1b; padding: 12px; border-radius: 8px; margin-bottom: 16px;">
                        {{ session('error') }}
                    </div>
                @endif
                                    ...
                                    <td class="text-center">
                                        @if ($user->wallet)
                                            <div class="wallet-info">
                                                <span
                                                    class="pill small wallet-balance">${{ number_format($user->wallet->balance, 2) }}</span>
                                                <button class="btn tiny orange"
                                                    onclick="editWallet({{ $user->id }}, {{ $user->wallet->balance }})">Edit</button>
                                            </div>
                                        @else
                                            <div class="wallet-info">
                                                <span class="pill small no-level">No Wallet</span>
                                                <button class="btn tiny green"
                                                    onclick="createWallet({{ $user->id }})">Create</button>
                                            </div>
                                        @endif
                                    </td>
                                    <td>
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form id="walletForm" method="post">
                    ...
                    <div class="field">
                        <label>Amount</label>
                        <input type="number" id="walletAmount" name="walletAmount" step="0.01" min="0"
                            required />
                    </div>
                    <div class="field">
