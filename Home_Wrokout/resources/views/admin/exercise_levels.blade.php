...
<div class="layout">
    ...
    <main class="main">
        <div class="topbar">
            <div class="page-title"><h1>Exercise Levels</h1></div>
        </div>
        ...
        <div class="page">
            <div class="add-button-container">
                ...
                <a href="{{ route('admin.exercise-levels.trashed') }}" class="btn gray" style="text-decoration: none; display: inline-block;">
                    🗑️ View Deleted Exercise Levels
                </a>
            </div>

            <div class="card add-form-container" id="addFormContainer" style="margin-bottom:14px">
                <div class="toolbar" style="margin-bottom:8px"><div class="muted">Add New Exercise Level</div></div>
            ...
            <div class="card">
                <div class="toolbar">
                    <div class="muted">All Exercise Levels (<span id="exerciseLevelsCount">{{ $exerciseLevels->count() }}</span> total)</div>
                    <div>
                ...
                <table class="table">
                    ...
                    <tbody>
                        ...
                        <tr class="exercise-level-row" data-level="{{ $el->level_id }}">
                            ...
                            <td>
                                <div class="actions">
                                    <button class="btn small gray" type="button" onclick="toggleEdit({{ $el->id }})">Edit</button>
                                    <form action="{{ route('admin.exercise-levels.delete', $el) }}" method="post" onsubmit="return confirm('Delete this exercise level?')" style="display:inline">
                                        @csrf
                        ...
                        <tr id="edit-row-{{ $el->id }}" style="display:none;background:#f8fafc" class="exercise-level-row" data-level="{{ $el->level_id }}">
                            <td colspan="7">
                                <form class="form-inline" action="{{ route('admin.exercise-levels.update', $el) }}" method="post">
                                    @csrf
                                    <div class="field">
                                        <label>Exercise</label>
                                        <select name="exercise_id" required>
                                            @foreach($exercises as $exercise)
                                                <option value="{{ $exercise->id }}" {{ $el->exercise_id == $exercise->id ? 'selected' : '' }}>{{ $exercise->name }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                    <div class="field">
                                        ...
                                        <select name="level_id" required>
                                            @foreach($levels as $level)
                                                <option value="{{ $level->id }}" {{ $el->level_id == $level->id ? 'selected' : '' }}>{{ $level->name }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                    <div class="field">
                                        <label>Calories</label>
                                        <input type="number" name="calories" min="1" value="{{ $el->calories }}" required />
                                    </div>
                                    <div class="field">
                                        <label>Reps</label>
                                        <input type="number" name="number_of_rips" min="1" value="{{ $el->number_of_rips }}" />
                                    </div>
                                    <div class="field">
                                        <label>Timer (seconds)</label>
                                        <input type="number" name="timer" min="1" value="{{ $el->timer ?? 30 }}" />
                                    </div>
                                    <div>
                                        <button class="btn" type="submit">Save</button>
                                    </div>
                                    <div>
                                        <button class="btn gray" type="button" onclick="toggleEdit({{ $el->id }})">Cancel</button>
                                    </div>
                                </form>
                            </td>
                        </tr>
