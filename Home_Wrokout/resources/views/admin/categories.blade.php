    ...
    <main class="main">
        ...
        <div class="page">
            ...
            <div class="add-button-container">
                ...
                <a href="{{ route('admin.categories.trashed') }}" class="btn gray" style="text-decoration: none; display: inline-block;">
            ...
            <div class="card add-form-container" id="addFormContainer">
                <div class="toolbar" style="margin-bottom:8px"><div class="muted">Add New Category</div></div>
                <form class="form-inline" action="{{ route('admin.categories.store') }}" method="post" enctype="multipart/form-data">
                    @csrf
                    <div class="field">
                        <label>Name</label>
            ...
            <div class="card">
                <div class="toolbar">
                    <div class="muted">All Categories (<span id="categoriesCount">{{ $categories->count() }}</span> total)</div>
                </div>
                    <table class="table">
                        <thead>
                            <tr>
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td>
                                    @if($category->image_path)
                                        <img class="img" src="{{ asset('storage/' . $category->image_path) }}" alt="{{ $category->name }}" />
                                    @else
                                        —
                                    @endif
                                </td>
                                <td>
                                    <div class="actions">
                                        <button class="btn small gray" type="button" onclick="toggleEdit({{ $category->id }})">Edit</button>
                            ...
                            <tr id="edit-row-{{ $category->id }}" style="display:none;background:#f8fafc">
                                <td colspan="5">
                                    <form class="form-inline" action="{{ route('admin.categories.update', $category) }}" method="post" enctype="multipart/form-data">
                                        @csrf
                                        <div class="field">
                                            <label>Name</label>
                                        ...
                                        <div class="field">
                                            ...
                                            @if($category->image_path)
                                                <div style="margin-top: 8px;">
                                                    <small class="muted">Current: {{ basename($category->image_path) }}</small>
                                                    <br>
                                                    <img class="img" src="{{ asset('storage/' . $category->image_path) }}" alt="{{ $category->name }}" style="width: 32px; height: 32px;" />
                                                </div>
                                            @endif
                                        </div>
                                        <div>
                                        ...
                                        <div>
                                            <button class="btn gray" type="button" onclick="toggleEdit({{ $category->id }})">Cancel</button>
                                        </div>
                                    </form>
                                </td>
                            </tr>
