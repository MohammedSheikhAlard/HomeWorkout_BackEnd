...
<div class="layout">
    ...
    <main class="main">
        <div class="topbar">
            <div class="page-title"><h1>Plan Days - {{ $plan->name }}</h1></div>
        ...
        <div class="page">
            ...
            <div class="card add-form-container" id="addFormContainer" style="margin-bottom:14px">
                <div class="toolbar" style="margin-bottom:8px"><div class="muted">Add New Day</div></div>
                <form class="form-inline" action="{{ route('admin.plans.days.store', $plan) }}" method="post">
                    ...
                    <div class="field">
                        ...
                        <input type="number" name="total_calories" min="0" value="0" readonly class="readonly-input" title="يتم حساب السعرات الحرارية تلقائياً من التمارين المضافة" />
                    </div>
                    <div class="field">
                ...
                <table class="table">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>
                                <form action="{{ route('admin.plans.days.toggle-rest', $day) }}" method="post">
                                    @csrf
                                    <button class="btn small {{ $day->is_rest_day ? 'red' : '' }}" type="submit">
                            ...
                            <td>
                                @if(!$day->is_rest_day)
                                    <a class="btn small" href="{{ route('admin.plans.day.exercises', $day) }}">View Exercises</a>
                                @else
                                    —
                                @endif
                            </td>
                            <td>
                                <div class="actions">
                                    <button class="btn small gray" type="button" onclick="toggleEdit({{ $day->id }})">Edit</button>
                                    <form action="{{ route('admin.plans.days.delete', $day) }}" method="post" onsubmit="return confirm('Delete this day?')" style="display:inline">
                                        @csrf
                        ...
                        <tr id="edit-row-{{ $day->id }}" style="display:none;background:#f8fafc">
                            <td colspan="6">
                                <form class="form-inline" action="{{ route('admin.plans.days.update', $day) }}" method="post">
                                    @csrf
                                    <div class="field">
                                        <label>Day Number</label>
                                        <input type="number" name="day_number" min="1" value="{{ $day->day_number }}" required />
                                    </div>
                                    <div class="field">
                                        <label>Total Calories</label>
                                        <input type="number" name="total_calories" min="0" value="{{ $day->total_calories }}" readonly class="readonly-input" title="يتم حساب السعرات الحرارية تلقائياً من التمارين المضافة" />
                                    </div>
                                    <div class="field">
                                        <label>Rest Day</label>
                                        <select name="is_rest_day" required>
                                            <option value="0" {{ $day->is_rest_day == 0 ? 'selected' : '' }}>No</option>
                                            <option value="1" {{ $day->is_rest_day == 1 ? 'selected' : '' }}>Yes</option>
                                        </select>
                                    </div>
                                    <div>
                                        <button class="btn" type="submit">Save</button>
                                    ...
                                    <div>
                                        <button class="btn gray" type="button" onclick="toggleEdit({{ $day->id }})">Cancel</button>
                                    </div>
                                </form>
                            </td>
                        </tr>
