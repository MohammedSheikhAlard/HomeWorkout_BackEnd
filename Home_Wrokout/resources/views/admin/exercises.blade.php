        ...
        <main class="main">
            ...
            <div class="page">
                <div class="add-button-container">
                    ...
                    <a href="{{ route('admin.exercises.trashed') }}" class="btn gray"
                ...
                <div class="card add-form-container" id="addFormContainer" style="margin-bottom:14px">
                    ...
                    <form class="form-inline" action="{{ route('admin.exercises.store') }}" method="post"
                                    ...
                                    <td>
                                        <div class="actions">
                                            <button class="btn small gray" type="button"
                                                onclick="toggleEdit({{ $exercise->id }})">Edit</button>
                                            <form action="{{ route('admin.exercises.delete', $exercise) }}"
                                                method="post" onsubmit="return confirm('Delete this exercise?')">
                                                @csrf
                                ...
                                <tr id="edit-row-{{ $exercise->id }}" style="display:none;background:#f8fafc">
                                    <td colspan="6">
                                        <form class="form-inline"
                                            action="{{ route('admin.exercises.update', $exercise) }}" method="post"
                                            enctype="multipart/form-data">
                                            @csrf
                                            <div class="field">
                                                <label>Name</label>
                                                ...
                                                <select name="category_id" required>
                                                    @foreach ($categories as $category)
                                                        <option value="{{ $category->id }}"
                                                            {{ $exercise->category_id == $category->id ? 'selected' : '' }}>
                                            ...
                                            <div>
                                                <button class="btn gray" type="button"
                                                    onclick="toggleEdit({{ $exercise->id }})">Cancel</button>
                                            </div>
                                        </form>
                                    </td>
                                </tr>
