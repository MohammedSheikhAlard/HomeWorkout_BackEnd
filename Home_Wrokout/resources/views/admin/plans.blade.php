        ...
        <main class="main">
            ...
            <div class="page">
                <div class="add-button-container">
                    ...
                    <a href="{{ route('admin.plans.trashed') }}" class="btn gray"
                ...
                <div class="card add-form-container" id="addFormContainer" style="margin-bottom:14px">
                    <div class="toolbar" style="margin-bottom:8px">
                        <div class="muted">Add New Plan</div>
                    </div>
                    <form class="form-inline" action="{{ route('admin.plans.store') }}" method="post">
                        ...
                        <div class="field">
                            ...
                            <input type="number" name="price" min="0" step="0.01"
                                placeholder="Leave empty for free plan" />
                        </div>
                        <div class="field">
                ...
                <div class="card">
                    <div class="toolbar">
                        <div class="muted">All Plans grouped by Level (<span
                                id="plansCount">{{ $plans->count() }}</span> total)</div>
                        <div>
                            <select id="levelFilter" onchange="filterPlans()"
                                style="border:1px solid var(--border);border-radius:8px;padding:8px;margin-right:10px;min-width:150px">
                        ...
                        <tbody>
                            @forelse($plans as $index => $plan)
                                <tr class="plan-row" data-level="{{ $plan->level_id }}">
                                    ...
                                    <td>
                                        <a class="btn small" href="{{ route('admin.plans.days', $plan) }}">View
                                            Days</a>
                                    </td>
                                    <td>
                                        <div class="actions">
                                            <button class="btn small gray" type="button"
                                                onclick="toggleEdit({{ $plan->id }})">Edit</button>
                                            <form action="{{ route('admin.plans.delete', $plan) }}" method="post"
                                                onsubmit="return confirm('Delete this plan? It will be soft-deleted.')">
                                                @csrf
                                ...
                                <tr id="edit-row-{{ $plan->id }}" style="display:none;background:#f8fafc"
                                    class="plan-row" data-level="{{ $plan->level_id }}">
                                    <td colspan="8">
                                        <form class="form-inline" action="{{ route('admin.plans.update', $plan) }}"
                                            method="post">
                                            @csrf
                                            <div class="field">
                                                <label>Name</label>
                                            ...
                                            <div class="field">
                                                <label>Days to Train</label>
                                                <input type="number" name="number_of_day_to_train" min="1"
                                                    value="{{ $plan->number_of_day_to_train }}" required />
                                            </div>
                                            <div class="field">
                                                <label>Level</label>
                                                <select name="level_id" required>
                                                    @foreach ($levels as $level)
                                                        <option value="{{ $level->id }}"
                                                            {{ $plan->level_id == $level->id ? 'selected' : '' }}>
                                                            {{ $level->name }}</option>
                                                    @endforeach
                                                </select>
                                            </div>
                                            ...
                                            <div>
                                                <button class="btn gray" type="button"
                                                    onclick="toggleEdit({{ $plan->id }})">Cancel</button>
                                            </div>
                                        </form>
                                    </td>
                                </tr>
